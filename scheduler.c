...
void update_run_time(thread_info_t *info) {
        ...
        clock_gettime(0,&info->suspend_time);
        long result = time_difference(&info->suspend_time, &info->resume_time);
        info->run_time = info->run_time + result;
}
...
void update_wait_time(thread_info_t *info) {
        /* DONE: implement this function */
        clock_gettime(0,&info->resume_time);
        long result = time_difference(&info->resume_time, &info->suspend_time);
        info->wait_time = info->wait_time + result;
}
...
static void resume_worker(thread_info_t *info)
{
	...
	/*
	 * DONE: signal the worker thread that it can resume 
	 */
...
void cancel_worker(thread_info_t *info)
{
	...
	/* DONE: send a signal to the thread, telling it to kill itself*/
...
static void suspend_worker(thread_info_t *info)
{
	...
	/* DONE: Update quanta remaining. */
	info->quanta = info->quanta - 1;
	
	/* DONE: decide whether to cancel or suspend thread */
	if(info->quanta >= 1) {
	  /*
	   * Thread still running: suspend.
	   * DONE: Signal the worker thread that it should suspend.
	   */
...
void setup_sig_handlers() {
	...
	sigalrm.sa_handler = timer_handler;
	...
	sigterm.sa_handler = cancel_thread;
	...
	sigusr1.sa_handler = suspend_thread;
...
static void create_workers(int thread_count, int *quanta)
{
	...
	for (i = 0; i < thread_count; i++) {
		...
		/* DONE: initialize the time variables for each thread for performance evalution*/
		struct timespec start;
		info->run_time = 0L;
		info->wait_time = 0L;
		clock_gettime(0, &start);
		info->suspend_time = start;
